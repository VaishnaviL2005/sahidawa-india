...
export default function MapView() {
  ...
  if (!userLocation || loading) return <div className="text-center p-8">Loading map...</div>;
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-medium border ${showPharmacies ? 'bg-green-600 text-white' : 'bg-white text-green-600 border-green-600'}`}
        >
          🟢 Pharmacies
        </button>
        <button
          onClick={() => setShowAsha(a => !a)}
          className={`px-4 py-2 rounded-full text-sm font-medium border ${showAsha ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border-blue-600'}`}
        >
          🔵 ASHA Workers
        </button>
      </div>
      ...
      <MapContainer center={userLocation} zoom={13} style={{ height: '500px', width: '100%' }}>
        ...
        {showPharmacies && pharmacies.map(p => (
